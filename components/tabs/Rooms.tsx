  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 18, marginBottom: 28 }}>
        {rooms.map((r) => (
          <button key={r.id} onClick={() => openRoom(r)} className="card-clean" style={{ background: "var(--bg-elev)", padding: 14, textAlign: "left", display: "block" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginTop: 2 }}>
              <span style={{ fontSize: 12, color: "var(--ink-mute)" }}>{r.bed} bed · sleeps {r.sleeps}</span>
              <span style={{ fontSize: 12, color: "var(--accent)" }}>{r.bookedThisWeek} nights booked</span>
            </div>
